...
struct slicer_t {
  offset_t begin, end;
  constexpr util_attrs explicit operator bool() const {
...
struct md_uview {
  ...
  template<std::size_t pos>
  constexpr util_attrs std::enable_if_t<(pos >= 0 && pos < DIMS), md_uview<T, DIMS>>
  slice(slicer_t pair) const {
    return slice<pos>(pair.begin, pair.end);
  }
...
void util_attrs copy(const md_view<T, DIMS> &dst, const md_view<const T, DIMS> &src, Memcpy cp) {
  assert(dst.shape == src.shape);
  cp(dst.data, src.data, dst.size() * sizeof(T));
}

template<class T, std::size_t DIMS, class Memcpy>
void util_attrs copy(const md_view<T, DIMS> &dst, const md_view<T, DIMS> &src, Memcpy cp) {
...
template<class T, std::size_t DIMS, class Memcpy>
void util_attrs copy(const md_uview<T, DIMS> &dst, const md_uview<const T, DIMS> &src, Memcpy cp) {
  detail::copy_impl(dst, src, cp);
}

template<class T, std::size_t DIMS, class Memcpy>
void util_attrs copy(const md_uview<T, DIMS> &dst, const md_uview<T, DIMS> &src, Memcpy cp) {
